...
PCA_plot <-ggplot(data = scores, aes(x = PC1, y = PC2, color = Neophyte)) +
  ...
  theme_pubr() +
  xlab(paste("PC1 (", round(apply(scores[1], 2,var)/sum(PCA$sdev^2)*100, 1), "%)", sep = "")) +
  ylab(paste("PC2 (", round(apply(scores[2], 2,var)/sum(PCA$sdev^2)*100, 1), "%)",sep = "")) +
...
MDL<- lmer(impact ~ PC1+ PC1: Neophyte+ PC2+PC2: Neophyte + Neophyte+ growth+ (1|family/genus), general2)
...
emmeans::emmeans(MDL, pairwise ~growth)
```
...
test$x <- as.factor(test$x)
general2$Neophyte <- as.facter(general2$Neophyte)

...
fullPlotData <- read_csv("fullPlotData_EUNIS.csv", show_col_types = FALSE)
...
```{r}
# Load impact data
fileName <- paste("Impact2_new70%_ENS0_reldiff_PCA.csv")
...
cor_extra<-cor(impact[impact$Neophyte=="extra", c(2,9, 22:27)],  use= "pairwise.complete.obs", method="pearson")
cor_intra <-cor(impact[impact$Neophyte=="intra", c(2,9, 22:27)],  use="pairwise.complete.obs", method="pearson")
cor_native <-cor(impact[impact$Neophyte=="native", c(2,9, 22:27)],  use="pairwise.complete.obs", method="pearson")
cor <- cor(impact[, c(2,9, 22:27)],  use="pairwise.complete.obs", method="pearson")
```
...
```{r}
# change crops 
crops <- eva_names[grepl("crop ", eva_names$name, ignore.case = TRUE),]
eva_names$name <- gsub("crop vineyard","Vitis vinifera", eva_names$name)
eva_names$name <- gsub("crop barley","Hordeum vulgare", eva_names$name)
eva_names$name <- gsub("crop maize","Zea mays", eva_names$name)
eva_names$name <- gsub("crop wheat","Triticum aestivum", eva_names$name)
eva_names$name <- gsub("crop potato","Solanum tuberosum", eva_names$name)
eva_names$name <- gsub("crop hops","Humulus lupulus", eva_names$name)
eva_names$name <- gsub("crop pea","Pisum sativum", eva_names$name)
eva_names$name <- gsub("crop digitalis","Digitalis purpurea", eva_names$name)
eva_names$name <- gsub("crop tomato","Solanum lycopersicum", eva_names$name)
eva_names$name <- gsub("crop currant","Ribes rubrum", eva_names$name)
eva_names$name <- gsub("crop cabbage","Brassica oleracea", eva_names$name)
eva_names$name <- gsub("crop bean","Phaseolus vulgaris", eva_names$name)
eva_names$name <- gsub("crop onion","Allium cepa", eva_names$name)
eva_names$name <- gsub("crop asparagus","Asparagus officinalis", eva_names$name)
eva_names$name <- gsub("crop spelt","Triticum aestivum", eva_names$name)
eva_names$name <- gsub("crop carrot","Daucus carota", eva_names$name)
eva_names$name <- gsub("crop rhubarb","Rheum rhabarbarum", eva_names$name)
eva_names$name <- gsub("crop clover","Trifolium repens", eva_names$name)
eva_names$name <- gsub("crop tobacco","Nicotiana tabacum", eva_names$name)
eva_names$name <- gsub("crop zucchini","Cucurbita pepo", eva_names$name)
eva_names$name <- gsub("crop parsley","Petroselinum crispum", eva_names$name)
eva_names$name <- gsub("crop gladiolus","Gladiolus grandiflorus", eva_names$name)
eva_names$name <- gsub("crop salsify","Tragopogon porrifolius", eva_names$name)
eva_names$name <- gsub("crop radish","Raphanus sativus", eva_names$name)
eva_names$name <- gsub("crop lettuce","Lactuca sativa", eva_names$name)
eva_names$name <- gsub("crop buckwheat","Fagopyrum esculentum", eva_names$name)
...
```{r}
# Change names FT
colnames(impact)[22:28]<- c("LA", "N", "LMA","H","SM","SSD","number")
colnames(impact)[35:41]<- c("Sev","Sev_herb","Freq","Freq_herb","Gras","Mow","Soil") 
...
MDL<- lmer(impact ~-1+ SSD: Neophyte+ LMA: Neophyte+ H: Neophyte+ LA: Neophyte+ 
             SM: Neophyte+ N: Neophyte+ Neophyte +(1|Family/Genus), 
           general)
summary(MDL)
...
#
library(ggeffects)
plot(ggpredict(MDL,terms= c("SSD", "Neophyte") ))

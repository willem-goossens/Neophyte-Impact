...
growth<- ggplot(test, aes(x= factor(x, level= c("herb","graminoid","tree","shrub","other")), predicted))+
...
test$x <- as.factor(test$x)
test$x
growth<- ggplot(test, aes(x= factor(x, level= c("non-woody","semi-woody","woody")), predicted))+
...
```{r}
plot <- ggarrange( PCA_plot, growth, PC1, PC2, labels= c("a","b","c","d"), common.legend = TRUE, legend="bottom", align="hv", nrow=2, ncol=2)

...
## 3.3 Euro+Med
```{r}
eva$`Matched concept` <- gsub(" c.f."," cf.", eva$`Matched concept`)
eva$`Matched concept` <- gsub(" cfr."," cf.", eva$`Matched concept`)
...
sum(is.na(eva_names$euro))
# cf. --> Axmanova
eva_names$euro[is.na(eva_names$euro)] <- Euro$TaxonConcept[match(vegdata::taxname.simplify(gsub(" cf.","",eva_names$species[is.na(eva_names$euro)])),
                                                                 vegdata::taxname.simplify(Euro$TaxonName))]
# how many species now --> additional 497 found
sum(is.na(eva_names$euro))
...
unknown$species[is.na(unknown$species)]<- Euro$TaxonConcept[match(vegdata::taxname.simplify(gsub(" cf.","",unknown$name[is.na(unknown$species)])),
                                                                 vegdata::taxname.simplify(Euro$TaxonName, genus=F, epithet=T, rank=F))]
unknown$source[!is.na(unknown$species)]<- "ESy Matched"
# how many do we add
length(unique(unknown$species))
# we add our findings to eva --> additional 699 species found now
...
# how many do we add
length(unique(unknown$species))
# we add our findings to eva --> 38 more species
eva[is.na(eva$species),]<- unknown
...
## 3.5 Idoia file
```{r}
idoia <- readxl::read_excel("~/Doctoraat/Impact/Extra data/Species names/Idoia_check_taxa.xlsx")
...
if(fast){
  ...
  for(i in 1: length(eva_names$species)){
    # powo
    r<- search_powo(vegdata::taxname.abbr(gsub(" cf.","",eva_names$species[i])))
  ...
  #saveRDS(mybiglist, "~/Doctoraat/Impact/Extra data/Species names/species_powo_ESy_new.RData")
} else {
    # read all data
    mybiglist<- readRDS("~/Doctoraat/Impact/Extra data/Species names/species_powo_ESy_new.RData")
}
```
...
eva_names$source<- eva$source[match( eva_names$`Matched concept`, eva$`Matched concept`)]
eva_names |> group_by(source) |> summarise(n=n())
```
...
```{r}
# load file 
taxonNameCorrections <- read_csv("~/Doctoraat/Impact/Extra data/Species names/Irena_taxa_check_Christian.csv", show_col_types = FALSE)
...
## 5.3 Test Irena 2
```{r}
irena <- readxl::read_excel("~/Doctoraat/Impact/Extra data/Species names/Irena_taxa_update.xlsx")
...
## 6.1 Plants
```{r}
remove <- c("Campylophyllopsis calcarea","Pellia endiviifolia cf.","Hypanthium glabre","Mylia anomala cf.","Pellia endiviifolia cf.",
            "Campylophyllum calcareum cf.", "Campylophyllum calcareum cf.")
eva[eva$`Matched concept` %in% remove,]
eva <- eva[!eva$`Matched concept` %in% remove,]
...
if(fast){
  ...
  results <- lapply(species_list, check_species)
  results <- plyr::ldply(results)
  write_csv(results, "../Extra data/Species names/GBIF_plants.csv")
} else {
  results <- read_csv("../Extra data/Species names/GBIF_plants.csv", show_col_types = FALSE)
}
...
colnames(eva)
eva_needed <- eva[, c(6,12,13,14,15)]
head(eva_needed)
...
```{r}
# read data
#eva <- read_csv("../EVA Data/eva_ESy_subsp.csv", show_col_types = FALSE)
...
# save
write_csv(eva, "../EVA data/eva_ESy.csv")
#eva <- read_csv("../EVA data/eva_ESy.csv")
```
...
species_data <- data.frame(old = c( "crop vineyard", "crop barley", "crop maize", "crop wheat", "crop potato","crop hops", "crop pea", "crop digitalis",
                                    "crop tomato", "crop currant","crop cabbage", "crop bean", "crop onion", "crop asparagus", "crop spelt",
                                    "crop carrot", "crop rhubarb", "crop clover", "crop tobacco", "crop zucchini", "crop parsley", "crop gladiolus",
                                    "crop salsify", "crop radish", "crop lettuce","crop buckwheat"),
                          new = c("Vitis vinifera", "Hordeum vulgare", "Zea mays", "Triticum aestivum", "Solanum tuberosum", "Humulus lupulus", 
                                  "Pisum sativum", "Digitalis purpurea", "Solanum lycopersicum", "Ribes rubrum","Brassica oleracea", 
                                  "Phaseolus vulgaris", "Allium cepa", "Asparagus officinalis", "Triticum aestivum","Daucus carota", 
                                  "Rheum rhabarbarum", "Trifolium repens", "Nicotiana tabacum", "Cucurbita pepo", "Petroselinum crispum", 
                                  "Gladiolus grandiflorus", "Tragopogon porrifolius", "Raphanus sativus", "Lactuca sativa",
...
# count
eva_names <- eva |> group_by(`Matched concept`, name, source) |> summarise(n=n())
...
test <- eva_names |> group_by(`Matched concept`,name, source, genus) |> summarise(n=n())

test |> group_by(source) |> summarise(n=n())

test <- test[!test$source == "Plant",]
test |> group_by(source, genus) |> summarise(n=n())

test <- test[test$genus=="1",]

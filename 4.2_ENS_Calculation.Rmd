...
## 2.1 Prepare
```{r}
# we calculate the fisher value for species present among multiple layers
check <- unique(eva[, c(6,10,21)])
length(unique(eva$name))
y <-check[duplicated(check$name) | duplicated(check$name, fromLast = T),]
unique(y$name)

# not possible here yet, as doing so would merge some species that are different subspecies per plot. Will have to be done in every file seperately to be able to keep e.g. EIVE values that differ and have less of an impact. It is important to note that this file takes this into account by calculating the ENSO only on species names (so not subspecies removed through EUNIS-ESy)
...
eva[eva$PlotObservationID=="27146",]
fullPlotData[fullPlotData$PlotObservationID=="27146",]

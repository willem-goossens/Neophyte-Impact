...
```{r}
# how many observations have unknown taxon group
length(eva$species[eva$`Taxon group`=="Unknown"])
...
length(plant_list$YOUR_SEARCH[plant_list$GROUP =="Vascular plant"])

plant_list <- plant_list[!duplicated(plant_list) & plant_list$GROUP=="Vascular plant",]
plant_list<- plant_list[!is.na(plant_list$YOUR_SEARCH),]
...
```{r}
eva_names <- read.csv("eva_accepted_names_all.csv")
eva_names <- eva_names[(eva_names$old.name %in% eva$species & eva_names$PlotObservationID %in% eva$PlotObservationID),]

# all species are in boh lists, except the ones I removed myself.
 unique(eva$species[!eva$species %in% eva_names$old.name])
# we continue using eva_names
eva <- eva_names
colnames(eva)[5:6] <- c("Layer","Cover %")
```
...
Layersum=T
if(Layersum){
  eva <- eva |> group_by(PlotObservationID, species, old.name) |> summarise(`Cover %`= sum(`Cover %`))
}
...
eva[eva$PlotObservationID %in% totalCover$PlotObservationID[totalCover$totalCover> 20000],]
...
# check which is best name for EIVE matching
length(unique(eva$species[match(eva$species, eive$TaxonConcept)]))
length(unique(eva$old.name[match(eva$old.name, eive$TaxonConcept)]))

# we clearly also need to incorporate the old names
extra <- setdiff(unique(eva$old.name[match(eva$old.name, eive$TaxonConcept)]), unique(eva$species[match(eva$species, eive$TaxonConcept)]))

# redo analysis
eva$EIVEresM[eva$old.name %in% extra] <- eive$`EIVEres-M`[match(eva$old.name[eva$old.name %in% extra], eive$TaxonConcept)]
eva$EIVEresN[eva$old.name %in% extra] <- eive$`EIVEres-N`[match(eva$old.name[eva$old.name %in% extra], eive$TaxonConcept)]
eva$EIVEresR[eva$old.name %in% extra] <- eive$`EIVEres-R`[match(eva$old.name[eva$old.name %in% extra], eive$TaxonConcept)]
eva$EIVEresL[eva$old.name %in% extra] <- eive$`EIVEres-L`[match(eva$old.name[eva$old.name %in% extra], eive$TaxonConcept)]
eva$EIVEresT[eva$old.name %in% extra] <- eive$`EIVEres-T`[match(eva$old.name[eva$old.name %in% extra], eive$TaxonConcept)]

# niche widths
eva$EIVEnwM[eva$old.name %in% extra] <- eive$`EIVEres-M.nw3`[match(eva$old.name[eva$old.name %in% extra], eive$TaxonConcept)]
eva$EIVEnwN[eva$old.name %in% extra] <- eive$`EIVEres-N.nw3`[match(eva$old.name[eva$old.name %in% extra], eive$TaxonConcept)]
eva$EIVEnwR[eva$old.name %in% extra] <- eive$`EIVEres-R.nw3`[match(eva$old.name[eva$old.name %in% extra], eive$TaxonConcept)]
eva$EIVEnwL[eva$old.name %in% extra] <- eive$`EIVEres-L.nw3`[match(eva$old.name[eva$old.name %in% extra], eive$TaxonConcept)]
eva$EIVEnwT[eva$old.name %in% extra] <- eive$`EIVEres-T.nw3`[match(eva$old.name[eva$old.name %in% extra], eive$TaxonConcept)]
...
# check length DIV matching
length(unique(eva$species[match(eva$species, div$species)]))
length(unique(eva$old.name[match(eva$old.name, div$species)]))

# make list species
extra <- setdiff(unique(eva$old.name[match(eva$old.name, div$species)]),unique(eva$species[match(eva$species, div$species)]))

# redo for these species
eva$Disturbance.Severity[eva$old.name %in% extra] <- div$Disturbance.Severity[match(eva$old.name[eva$old.name %in% extra], div$species)]
eva$Disturbance.Severity.herblayer[eva$old.name %in% extra] <- div$Disturbance.Severity.herblayer[match(eva$old.name[eva$old.name %in% extra], div$species)]
eva$Disturbance.Frequency[eva$old.name %in% extra] <- div$Disturbance.Frequency[match(eva$old.name[eva$old.name %in% extra], div$species)]
eva$Disturbance.Frequency.herblayer[eva$old.name %in% extra] <- div$Disturbance.Frequency.herblayer[match(eva$old.name[eva$old.name %in% extra], div$species)]
eva$Grazing.Pressure[eva$old.name %in% extra] <- div$Grazing.Pressure[match(eva$old.name[eva$old.name %in% extra], div$species)]
eva$Mowing.Frequency[eva$old.name %in% extra] <- div$Mowing.Frequency[match(eva$old.name[eva$old.name %in% extra], div$species)]
eva$Soil.Disturbance[eva$old.name %in% extra] <- div$Soil.Disturbance[match(eva$old.name[eva$old.name %in% extra], div$species)]
...
# write_csv(fullPlotData, "fullPlotData_cover_all_layer_new.csv")
 
test <- read.csv("fullPlotEva_cover_all_layer_new.csv")
test2 <- read.csv("fullPlotEva_cover_all_layer.csv")

length(unique(test$old.name[!test$old.name %in% test2$species]))
# Make sure all eva data has a plot 
eva <- eva[eva$PlotObservationID %in% fullPlotData$PlotObservationID,]
# write_csv(eva, "fullPlotEva_cover_all_layer_new.csv")
```



```{r}
eva2 <- read_csv("fullPlotEva_cover_all_layer.csv", show_col_types = FALSE)

# species that are now included and not previously
extra <- unique(eva$old.name[!eva$old.name %in% eva2$species])
extra

unique(unknown$species[unknown$species %in% extra])

# which species do we have now and not previously and was not designated as unknown 
unique(extra[!extra %in% unknown$species])

# species that were included and not now
extra <- unique(eva2$species[!eva2$species %in% eva$old.name])
extra
# these are the species that we removed
```
...
# File for Irena
# Eva
fullPlotEva <- read_csv("fullPlotEva_cover_all_layer_new.csv", show_col_types = FALSE)
eva2<- fullPlotEva[,c("PlotObservationID","species","old.name")]
fullPlotData<- read_csv("fullPlotData_cover_all_layer_new.csv", show_col_types = FALSE)
fullPlot2<- fullPlotData[,c("PlotObservationID","Region")]

# Join and summarise
eva_country<- left_join(eva2, fullPlot2)
species_country<- eva_country |> group_by(Region) |> summarize(old.name=unique(old.name))

neophyte<- readxl::read_excel("../Neophyte-Impact/country_species-2024-01-30-IA-VK.xlsx", sheet="country_speciesWillem")

# these are species that are not in the previously made dataset (1389)
length(unique(neophyte$species[!neophyte$species %in% species_country$old.name]))
...
reducedEva <- eva |> select(PlotObservationID, species, old.name, `Cover %`)

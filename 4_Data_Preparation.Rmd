...
eva|> group_by(PlotObservationID) |> summarise(n= mean(log(eva$Disturbance.Severity), na.rm=T))
```


Join the result with the header data:
```{r}
fullPlotData<- subset(header, select= c(PlotObservationID, Country,Longitude, Latitude, Region, Dataset, `Relevé area (m²)`))
...
glm0 <- glm(numberOfVascularPlantSpecies ~ 
                    ...
                    DistSeverity.sqrt + I(DistSeverity.sqrt^2) +
                    DistFrequency.sqrt + I(DistFrequency.sqrt^2)

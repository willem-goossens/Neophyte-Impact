...
```{r}
dup_check <- highDiversityPlots[duplicated(highDiversityPlots[, c(5,9)]) | duplicated(highDiversityPlots[, c(5,9)], fromLast=T), ]

check <- eva[eva$PlotObservationID %in% dup_check$PlotObservationID, ]

list_species <-  check |> group_by(PlotObservationID) %>%
  summarise(species_set = paste(sort(unique(name)), collapse = ","), .groups = "drop")

remove <- which(duplicated(list_species$species_set))
length(remove)
fullPlotData <- fullPlotData[!fullPlotData$PlotObservationID %in%
                               list_species$PlotObservationID[remove],]
```
...
data <- data[!data$PlotObservationID %in% remove,]
...
# check also for plots with duplicate coordinates, date and species richnesses
dup <- data[duplicated(data[, c(8,35,9)]) | 
              duplicated(data[, c(8,35,9)], fromLast=T),]
# this is not feasible to do
dup <- dup[duplicated(dup[, c(9,10:19, 25:29,35)]) | 
              duplicated(dup[, c(9,10:19, 25:29,35)], fromLast=T),]
remove <- dup$PlotObservationID[duplicated(dup[, c(9, 10:19, 25:29,35)])]
length(remove)
fullPlotData <- fullPlotData[!fullPlotData$PlotObservationID %in% remove,]
data <- data[!data$PlotObservationID %in% remove,]
...
remove <- dup$PlotObservationID[duplicated(dup[, c(8,35,9)])| duplicated(dup[, c(8,35,9)], fromLast=T)]
length(remove)

test <- eva[eva$PlotObservationID %in% remove, c(1,6)]
test$all <- data$all[match(test$PlotObservationID, data$PlotObservationID)]


# Group by plot and collect species as sorted concatenated string
species_by_plot <- test %>%
  group_by(PlotObservationID, all) %>%
  summarise(species_set = paste(sort(unique(name)), collapse = ","), .groups = "drop")

# Find plots with identical species sets
duplicate_species_sets <- species_by_plot %>%
  group_by(species_set, all) %>%
  filter(n() > 1) %>%
  summarise(plots_with_same_species = list(PlotObservationID), .groups = "drop")

# Optional: see number of matching plots per group
duplicate_species_sets$plot_count <- sapply(duplicate_species_sets$plots_with_same_species, length)

# View result


# Step 3: Flatten the list and identify which PlotObservationIDs to remove
to_remove <- unlist(lapply(duplicate_species_sets$plots_with_same_species,
                           function(ids) ids[-1]))  # keep the first, remove the rest

# Step 4: Remove duplicates from the original `test` dataset
test <- test[!test$PlotObservationID %in% to_remove,]

dup <- dup[!dup$PlotObservationID %in% to_remove,]

length(to_remove)

fullPlotData <- fullPlotData[!fullPlotData$PlotObservationID %in% to_remove,]
```

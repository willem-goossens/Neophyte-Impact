...
not_species <- neophyte[!neophyte$species %in% eva_names$species,]
not_irena <- not_species[! not_species$species %in% eva_names$irena,]
# still 1449 not (1418)
not_not <- not_irena[!not_irena$species %in% eva_names$`Matched concept`,]
# 1354 not (1323)
...
# can be checked by merging with original header data
...
# some are additionally present in irena (205 left) -_> 223
...
# This leaves 202 (205) region species observations with no status assigned
not_not <- not_irena[!not_irena$`Matched concept` %in% neophyte$species &
                       is.na(not_irena$irena),]
species_country <- species_country[!is.na(species_country$n),]

species_assign <- species_country[is.na(species_country$Neophyte),]

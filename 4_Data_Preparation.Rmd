...
Layersum=F
if(Layersum){
  eva <- eva |> group_by(PlotObservationID, species, `Matched concept`, irena) |> summarise(`Cover %`= sum(`Cover %`))
} else {
  eva <- eva |> select(PlotObservationID, species, `Matched concept`, irena,`Cover %`)
...
# check names
eva$eive_name <- eive$TaxonConcept[match(eva$species, eive$TaxonConcept)]
...
# Second we add Irena data for these species
...
# check new names
eva$eive_name[eva$species %in% extra$species]  <- eive$TaxonConcept[match(eva$irena[eva$species %in% extra$species] , eive$TaxonConcept)]
length(eive$TaxonConcept[match(eva$irena[eva$species %in% extra_irena$species] , eive$TaxonConcept)])
new_names <- eva[is.na(eva$eive_name),]
...
no_eive <- unique(no_eive[, 2:4])

# checl against the length of eva names
eva_names <- unique(eva[,2:4])
...
# check names
eva$div_name <- div$species[match(eva$species, div$species)]
...
eva <- eva[eva$PlotObservationID %in% fullPlotData$PlotObservationID,]
fullPlotEva <- eva
#write_csv(eva, "fullPlotEva_euro.csv")
```
...
# File for Irena
# Eva
fullPlotEva2 <- read_csv("fullPlotEva_euro.csv", show_col_types = FALSE)
eva2<- fullPlotEva2[,c("PlotObservationID","species","irena", "Matched concept")]
fullPlotData2<- read_csv("fullPlotData_euro.csv", show_col_types = FALSE)
fullPlot2<- fullPlotData2[,c("PlotObservationID","Region")]

# eva names
eva_names2 <- unique(new[, c(6,12:13,3)])
eva_names3 <- unique(eva2[, c(2:4)])


# Join and summarise
eva_country<- left_join(eva2, fullPlot2)
species_country<- eva_country |> group_by(Region, species, irena, `Matched concept`) |> summarize(n=n())

# load neophyte data
neophyte<- readxl::read_excel("../Neophyte-Impact/country_species-2024-01-30-IA-VK.xlsx", sheet="country_speciesWillem")
neophyte <- neophyte[, c(2:3,5)]
neophyte$Neophyte[is.na(neophyte$Neophyte)] <- "native"

# join
species_country <- full_join(species_country, neophyte, by= c("species"="species", "Region"="Region"))
species_country$Neophyte[is.na(species_country$Neophyte)] <- 
  neophyte$Neophyte[match(species_country$irena[is.na(species_country$Neophyte)], neophyte$species)]
species_country$Neophyte[is.na(species_country$Neophyte)] <- 
  neophyte$Neophyte[match(species_country$`Matched concept`[is.na(species_country$Neophyte)], neophyte$species)]

# not
not <- species_country[is.na(species_country$Neophyte),]
not[!duplicated(not$species),]

# check whether all our new species are in the old data (2715 not)
not_species <- neophyte[!neophyte$species %in% eva_names3$species,]
not_irena <- not_species[! not_species$species %in% eva_names3$irena,]
# still 1449 not
not_not <- not_irena[!not_irena$species %in% eva_names3$`Matched concept`,]
not_not <- not_not[!duplicated(not_not$species),]

# the species not present here and yes previously were removed due to eg date etc
unique(not_not$species[!not_not$species %in% eva_names$irena])
unique(not_not$species[!not_not$species %in% eva_names2$irena])
unique(not_not$species[!not_not$species %in% eva_names3$species])
```


```{r}
# opposite (1112 are in our new database and not previous one)
species <- unique(species_country$species[species_country$species %in% neophyte$species])
# remove species that are present
not_species <- species_country[!species_country$species %in% species,]
taxonNameCorrections$statusEurope[taxonNameCorrections$species %in% not_species$species] 
# some are additionally present in irena (205 left)
not_irena <- not_species[!not_species$irena %in% neophyte$species,]
unique(not_irena$irena) 
# 14 species were present in Irena but not anymore 
# check status in original file --> all native so no problem
taxonNameCorrections$statusEurope[taxonNameCorrections$species %in% unique(not_irena$irena)] 
# check all species with NA
length(unique(not_irena$`Matched concept`[is.na(not_irena$irena)]))
# or matched concept
not_not <- not_irena[not_irena$`Matched concept` %in% neophyte$species,]
# 2 species were matched using the Matched concept
# This leaves 202 region species observations with no status assigned
not_not <- not_irena[!not_irena$`Matched concept` %in% neophyte$species &
                       is.na(not_irena$irena),]

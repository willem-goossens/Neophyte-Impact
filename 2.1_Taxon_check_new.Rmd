...
eva_names$`Taxon group`[eva_names$`Matched concept` %in% no_dup$`Matched concept`] <- 
  no_dup$`Taxon group`[match(eva_names$`Matched concept`[eva_names$`Matched concept` %in% no_dup$`Matched concept`],no_dup$`Matched concept`)]
...
if(fast){
  ...
  for(i in 1: length(eva_names$species)){
    # powo
    r<- search_powo(eva_names$species[6550])
    ...
    # read all data
    mybiglist<- readRDS("species_powo.RData")
}
```
  ...
  if(!all(is.na(mybiglist[[i]]))){
    ...
    if(any(mybiglist[[i]]$accepted==T)){
      accepted <- add_row(accepted, new_name= mybiglist[[i]]$name[mybiglist[[i]]$accepted==T], old_name =eva_names$species[i],  
                          number= i, taxon= eva_names$taxon[i])
    } else {
      accepted <- add_row(accepted, new_name= NA, old_name =eva_names$species[i] ,  number= i, taxon=eva_names$taxon[i])
    }
  } else {
      accepted <- add_row(accepted, new_name= NA,old_name = eva_names$species[i],  number= i, taxon= eva_names$taxon[i] )
  } 
...
remove <- c("unknown* species","Grass species","? species", "Mos ?", "Unknown species", "Moss species","Fungus species", 
            ...
            "Gray lichen", "Small grass", "Spec species", "Tuberous plant")
...
# add column
eva$irena <- taxonNameCorrections$species[match(tolower(eva$`Matched concept`),
                                                    tolower(taxonNameCorrections$Matched.concept))]
...
remove <- c("bryophytes* species","Hepaticopsida species","Nostoc species", "Bryophyta sp","Nostocaceae species","Algae species",
            "Unknown species 4","Unknown sp","Algae* species","Poaceae, leguminosae...","crustaceous lichens","Biota species",
            "Parmeliaceae species")
eva <- eva[!eva$`Matched concept`%in% remove,]
```
...
# check unique names we will assign
...
# check whether we have the dupplicated species from the accepted names
...
new <- read_csv("eva_euro_own.csv")

# here present
check<- eva[!eva$species %in% new$species,]
# new present
check<- new[!new$species %in% eva$species,]

# check difference
setdiff(new, eva[,1:12])
setdiff(eva[,1:12], new)
```

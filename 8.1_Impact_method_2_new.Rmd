...
```{r, meassage=F}
# The name of the first species (to test the function)
plantName = allPlants$names[9]
allPlants[allPlants$names=="Achillea millefolium aggr.",]
plantStatus= allPlants$Neophyte[9]
...
TukeyHSD(result)
  ...
  z<- aligned.rank.transform(RelDiff ~ Neophyte * class, data = x)
  x$class <- as.factor(x$class)
  x$Neophyte <- as.factor(x$Neophyte)
  z<- art(RelDiff ~ Neophyte * class, data = x)
  z
  
  kruskal.test(RelDiff ~ Neophyte + class, data = x)
  
  kruskal.test(RelDiff ~ Neophyte + class, data = x)

  pairwise.wilcox.test(x$RelDiff,x$Neophyte,p.adjust.method = "BH")
  FSA::dunnTest(RelDiff ~ Neophyte , data = x, method = "bh")
 }
...
plot <- ggarrange(very_low_plot, low_plot, medium_plot, high_plot,
                  very_high_plot, nrow=3, ncol=2, labels= c("a","b", "c","d","e"),
...
p<-ggplot(x, aes(x=Neophyte, y= RelDiff, group = Species,
  ...
  theme_pubr()+
  stat_compare_means(comparisons=my_comparisons, label= "p.signif", label.y = c(400,410, 420))+
  stat_summary(fun= "mean",
               geom = "point", aes(group= Neophyte), size=3)+
  scale_colour_manual(values=c("#1E88E5", "#FFC107", "#004D40"))+
  scale_fill_manual(values = c("#1E88E5", "#FFC107", "#004D40")) +

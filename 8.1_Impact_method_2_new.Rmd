...
if(fast) {
  ...
  if(native_intra_analysis){
  plotsPerSpecies <- read.csv("../Results/speciesDominance_1980_native_intra.csv")
  }else{
  plotsPerSpecies<- read.csv("../Results/speciesDominance.csv")
  }
}
```
...
```{r}
x <- read_csv('I:/Impact_1980_new_again.csv')
# remove genus data
...
p<-ggplot(Dataset, aes(x=Neophyte, y= RelDiff, group = Species,
    ...
    annotate("text", x = 2, y = max5 + max5/10, label = "b", size = 4, vjust = 0.5, hjust = 0.5, alpha=1)+
    annotate("text", x = 3, y = max5 + max5/10, label = "a", size = 4, vjust = 0.5, hjust = 0.5, alpha=1)+

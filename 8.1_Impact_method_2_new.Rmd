...
fast <- T
if(fast) {
  fullPlotData <- fullPlotData[runif(length(fullPlotData$PlotObservationID)) > 0.95,]
...
if(fast) {
  ...
  plotsPerNatives <- data.frame(names = character(), numberOfPlots = integer(), Neophyte= character())
  reducedNative<- reducedEva[reducedEva$Neophyte=="native",]
  uniqueNatives <- unique(reducedNative$species)

  # Count in how many plots a native species occurs
  begin<- Sys.time()
plotsPerNatives<-foreach(i= 1:length(uniqueNatives), .combine='rbind', .packages=c("dplyr","mgcv")) %dopar% {
   tmp <- reducedNative$PlotObservationID[reducedNative$species == uniqueNatives[i]]
    names=uniqueNatives[i]
    numberOfPlots= length(unique(tmp))
    plotsPerNatives[i,]<- c(names, numberOfPlots,Neophyte= "native")
}
  plotsPerNatives<- as.data.frame(plotsPerNatives)
  plotsPerNatives<-setNames(plotsPerNatives, c("names", "numberOfPlots"))
  row.names(plotsPerNatives)<- c(1: length(uniqueNatives))
  
  end<- Sys.time()
  round(end-begin)

  plotsPerIntra <- data.frame(names = character(), numberOfPlots = integer(), Neophyte= character())
  reducedNative<- reducedEva[reducedEva$Neophyte=="intra",]
  uniqueIntra <- unique(reducedNative$species)

  # Count in how many plots an intra European species occurs
  begin<- Sys.time()
plotsPerIntra<-foreach(i= 1:length(uniqueIntra), .combine='rbind', .packages=c("dplyr","mgcv")) %dopar% {
   tmp <- reducedNative$PlotObservationID[reducedNative$species == uniqueIntra[i]]
    names=uniqueIntra[i]
    numberOfPlots= length(unique(tmp))
    plotsPerIntra[i,]<- c(names, numberOfPlots, Neophyte="intra")
}
  plotsPerIntra<- as.data.frame(plotsPerIntra)
  plotsPerIntra<-setNames(plotsPerIntra, c("names", "numberOfPlots"))
  row.names(plotsPerIntra)<- c(1: length(uniqueIntra))
  
  plotsPerExtra <- data.frame(names = character(), numberOfPlots = integer(), Neophyte= character())
  reducedNative<- reducedEva[reducedEva$Neophyte=="extra",]
  uniqueExtra <- unique(reducedNative$species)

  # Count in how many plots an extra European species occurs
  begin<- Sys.time()
plotsPerExtra<-foreach(i= 1:length(uniqueExtra), .combine='rbind', .packages=c("dplyr","mgcv")) %dopar% {
   tmp <- reducedNative$PlotObservationID[reducedNative$species == uniqueExtra[i]]
    names=uniqueExtra[i]
    numberOfPlots= length(unique(tmp))
    plotsPerExtra[i,]<- c(names, numberOfPlots, Neophyte= "extra")
}
  plotsPerExtra<- as.data.frame(plotsPerExtra)
  plotsPerExtra<-setNames(plotsPerExtra, c("names", "numberOfPlots"))
  row.names(plotsPerExtra)<- c(1: length(uniqueExtra))
  
  parallel::stopCluster(cl = my.cluster)
} else {
  plotsPerSpecies2 <- read_csv("plotsPerSpeciesInEurope.csv")
}

plotsPerSpecies<- rbind(plotsPerNatives, plotsPerIntra, plotsPerExtra)
plotsPerSpecies$numberOfPlots <- as.numeric(plotsPerSpecies$numberOfPlots)
colnames(plotsPerSpecies)[3]<- "Neophyte"
```
...
base <- bam(model,family = poisson, data = tmpFullPlotData,  method = 'fREML',  discrete=TRUE, nthreads=4)
  ...
  res <- add_row(res, taxa = plantName, Estimate = baseModel$p.table[2,1], StdErr = baseModel$p.table[2,2], zValue = baseModel$p.table[2,3], pr = baseModel$p.table[2,4], numberOfPlots = length(plotsWherePlantOccurs$PlotObservationID), Neophyte= allPlants$Neophyte[i])
  res
}
```
...
# 3 PLOT
```{r}
ggplot(x, aes(x=Neophyte, y= Estimate, color=Neophyte))+geom_violin()+

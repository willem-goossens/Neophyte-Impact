...
if(fast) {
  ...
  if(native_intra_analysis){
  plotsPerSpecies <- read.csv("../Results/speciesDominance_1980.csv")
  }else{
  plotsPerSpecies<- read.csv("../Results/speciesDominance_1980.csv")
  }
}
...
PCA<- prcomp(fullPlotData[, c(16:20, 26,28:30, 41,43,44)  ], scale=T)
cor.test(fullPlotData$chelsaT, fullPlotData$EIVEresT)
...
## 2.1 Function 
```{r, meassage=F}
which(allPlants$names=="Rumex patientia aggr.")
# The name of the first species (to test the function)
plantName = allPlants$names[5212]
plantStatus= allPlants$neophyte[5212]
...
computePlantImpact <- function(plantName, plantStatus) {
  ...
  # Copy the full dataset without aberrant totalCover
  tmpFullPlotData <- fullPlotData %>% filter(totalCover <=500)
...
sum(res$RelDiff*res$n/res$numberOfPlots)
```
...
impact <- x
y <- read_csv('I:/Impact_1980_final.csv')
all.equal(x, y)
}
```
...
```{r}
x <- read_csv('I:/Impact_1980_final.csv')
# remove genus data
...
phylo <- read_csv("../Extra data/Species names/phylo.csv",show_col_types = FALSE)
...
assign(paste("Result",gsub(" ","_",index), sep = ""), result)

}

ResultVery_low_cover |> group_by(contrast) |> summarise(sign = sum(p.value < 0.05)/length(p.value))
ResultLow_cover |> group_by(contrast) |> summarise(sign = sum(p.value < 0.05)/length(p.value))
ResultMedium_cover |> group_by(contrast) |> summarise(sign = sum(p.value < 0.05)/length(p.value))
ResultHigh_cover |> group_by(contrast) |> summarise(sign = sum(p.value < 0.05)/length(p.value))
ResultVery_high_cover |> group_by(contrast) |> summarise(sign = sum(p.value < 0.05)/length(p.value))
ResultExtremely_high_cover |> group_by(contrast) |> summarise(sign = sum(p.value < 0.05)/length(p.value))
 
model <- lmer(RelDiff ~ Neophyte + (1|family/genus), x[x$class=="0%-1%",])
emmeans::emmeans(model, pairwise ~ Neophyte)
plot(emmeans::emmeans(model, pairwise ~ Neophyte))
model <- lmer(RelDiff ~ Neophyte + (1|family/genus), x[x$class=="1%-10%",])
emmeans::emmeans(model, pairwise ~ Neophyte)
plot(emmeans::emmeans(model, pairwise ~ Neophyte))
model <- lmer(RelDiff ~ Neophyte + (1|family/genus), x[x$class=="10%-30%",])
emmeans::emmeans(model, pairwise ~ Neophyte)
plot(emmeans::emmeans(model, pairwise ~ Neophyte))
model <- lmer(RelDiff ~ Neophyte + (1|family/genus), x[x$class=="30%-50%",])
emmeans::emmeans(model, pairwise ~ Neophyte)
plot(emmeans::emmeans(model, pairwise ~ Neophyte))
model <- lmer(RelDiff ~ Neophyte + (1|family/genus), x[x$class=="50%-70%",])
emmeans::emmeans(model, pairwise ~ Neophyte)
plot(emmeans::emmeans(model, pairwise ~ Neophyte))
model <- lmer(RelDiff ~ Neophyte + (1|family/genus), x[x$class=="70%-100%",])
emmeans::emmeans(model, pairwise ~ Neophyte)
plot(emmeans::emmeans(model, pairwise ~ Neophyte))
```
...
# create title
title <- paste(index, index2)
class <- paste("]",cover_class[i], "]", sep="")

if(i %%2 ==0){
  min2 <- min(Dataset$RelDiff)
  max1 <- max(Dataset$RelDiff)
}else{
  max1 <- max(Dataset$RelDiff)
  min2 <- min(Dataset$RelDiff)
}
...
p<-ggplot(Dataset, aes(x=Neophyte, y= RelDiff, group = Species,
    ...
    annotate("text", x = 1, y = max1 + max1/10,label = "b", size = 4, vjust = 0.5, hjust = 0.5, alpha=1) +
    annotate("text", x = 2, y = max1 + max1/10, label = "b", size = 4, vjust = 0.5, hjust = 0.5, alpha=1)+
    annotate("text", x = 3, y = max1 + max1/10, label = "a", size = 4, vjust = 0.5, hjust = 0.5, alpha=1)+
    annotate("text", x=1, y=max1 + max1/5, 
             label= paste("n=",length(Dataset$Estimate[Dataset$Neophyte=="extra"])), 
             vjust = 0, hjust = 0.5, size=4)+
    annotate("text", x=2, y=max1 + max1/5, 
             label= paste("n=",length(Dataset$Estimate[Dataset$Neophyte=="intra"])), 
             vjust = 0, hjust = 0.5, size=4)+
    annotate("text", x=3, y=max1 + max1/5, 
             label= paste("n=",length(Dataset$Estimate[Dataset$Neophyte=="native"])), 
             vjust = 0, hjust = 0.5, size=4)+
    annotate("text", x=1, y=min2 + min2/5, 
             label= paste("mean=",format(round(mean(Dataset$RelDiff[Dataset$Neophyte=="extra"]), digits=2),  nsmall=2)), 
             vjust = 0, hjust = 0.5, size=4)+
    annotate("text", x=2,y=min2 +min2/5, 
             label= paste("mean=",format(round(mean(Dataset$RelDiff[Dataset$Neophyte=="intra"]), digits=2),  nsmall=2)), 
             vjust = 0, hjust = 0.5, size=4)+
    annotate("text", x=3, y=min2 + min2/5, 
             label= paste("mean=", format(round(mean(Dataset$RelDiff[Dataset$Neophyte=="native"]), digits= 2), nsmall=2)), 
             vjust = 0, hjust = 0.5, size=4)+
    ...
    scale_y_continuous(limits=c(min2+ min2/5, max1+max1/5))+
...
p<-ggplot(Dataset, aes(x=Neophyte, y= RelDiff, group = Species,
    ...
    theme(legend.position = "none")+
    theme(legend.text= element_text(size=12),legend.title =element_text(size=14))+
    annotate("text", x = 1, y = max1 + max1/10,label = "c", size = 4, vjust = 0.5, hjust = 0.5, alpha=1) +
    annotate("text", x = 2, y = max1 + max1/10, label = "b", size = 4, vjust = 0.5, hjust = 0.5, alpha=1)+
    annotate("text", x = 3, y = max1 + max1/10, label = "a", size = 4, vjust = 0.5, hjust = 0.5, alpha=1)+
    annotate("text", x=1, y=max1 + max1/5, 
             label= paste("n=",length(Dataset$Estimate[Dataset$Neophyte=="extra"])), 
...
if(i %%2 ==0){
  min4 <- min(Dataset$RelDiff)
}else{
  max3 <- max(Dataset$RelDiff)
}
...
p<-ggplot(Dataset, aes(x=Neophyte, y= RelDiff, group = Species,
    ...
    theme(legend.position = "none")+
    theme(legend.text= element_text(size=12),legend.title =element_text(size=14))+
    annotate("text", x = 1, y = max3 + max3/10,label = "b", size = 4, vjust = 0.5, hjust = 0.5, alpha=1) +
    annotate("text", x = 2, y = max3 + max3/10, label = "ab", size = 4, vjust = 0.5, hjust = 0.5, alpha=1)+
    annotate("text", x = 3, y = max3 + max3/10, label = "a", size = 4, vjust = 0.5, hjust = 0.5, alpha=1)+
    annotate("text", x=1, y=max3 + max3/5, 
             label= paste("n=",length(Dataset$Estimate[Dataset$Neophyte=="extra"])), 
             vjust = 0, hjust = 0.5, size=4)+
    annotate("text", x=2, y=max3 + max3/5, 
             label= paste("n=",length(Dataset$Estimate[Dataset$Neophyte=="intra"])), 
             vjust = 0, hjust = 0.5, size=4)+
    annotate("text", x=3, y=max3 + max3/5, 
             label= paste("n=",length(Dataset$Estimate[Dataset$Neophyte=="native"])), 
             vjust = 0, hjust = 0.5, size=4)+
    annotate("text", x=1, y=min4 + min4/5, 
             label= paste("mean=",format(round(mean(Dataset$RelDiff[Dataset$Neophyte=="extra"]), digits=2),  nsmall=2)), 
             vjust = 0, hjust = 0.5, size=4)+
    annotate("text", x=2,y=min4 +min4/5, 
             label= paste("mean=",format(round(mean(Dataset$RelDiff[Dataset$Neophyte=="intra"]), digits=2),  nsmall=2)), 
             vjust = 0, hjust = 0.5, size=4)+
    annotate("text", x=3, y=min4 + min4/5, 
             label= paste("mean=", format(round(mean(Dataset$RelDiff[Dataset$Neophyte=="native"]), digits= 2), nsmall=2)), 
             vjust = 0, hjust = 0.5, size=4)+
    #labs(subtitle = "Significance level: NS")+
    ylab("Relative impact")+
    ...
    theme(plot.title= element_text(hjust=0.01),
      plot.subtitle = element_text( hjust = 0.01))+
    scale_y_continuous(limits=c(min4+ min4/5, max3+max3/5))+
    scale_x_discrete(labels=c("Extra-\nEuropean", "Intra-\nEuropean", "Native", "Native\nalien elsewhere"))
...
p<-ggplot(Dataset, aes(x=Neophyte, y= RelDiff, group = Species,
    ...
    theme(legend.position = "none")+
    theme(legend.text= element_text(size=12),legend.title =element_text(size=14))+
    #annotate("text", x = 1, y = max5 + max5/10,label = "b", size = 4, vjust = 0.5, hjust = 0.5, alpha=1) +
    #annotate("text", x = 2, y = max5 + max5/10, label = "a", size = 4, vjust = 0.5, hjust = 0.5, alpha=1)+
    #annotate("text", x = 3, y = max5 + max5/10, label = "b", size = 4, vjust = 0.5, hjust = 0.5, alpha=1)+
    ...
    annotate("text", x=3, y=max5 + max5/5, 
             label= paste("n=",length(Dataset$Estimate[Dataset$Neophyte=="native"])), 
             vjust = 0, hjust = 0.5, size=4)+
    annotate("text", x=1, y=min6 + min6/5, 
             label= paste("mean=",format(round(mean(Dataset$RelDiff[Dataset$Neophyte=="extra"]), digits=2),  nsmall=2)), 
             vjust = 0, hjust = 0.5, size=4)+
    annotate("text", x=2,y=min6 +min6/5, 
             label= paste("mean=",format(round(mean(Dataset$RelDiff[Dataset$Neophyte=="intra"]), digits=2),  nsmall=2)), 
             vjust = 0, hjust = 0.5, size=4)+
    annotate("text", x=3, y=min6 + min6/5, 
             label= paste("mean=", format(round(mean(Dataset$RelDiff[Dataset$Neophyte=="native"]), digits= 2), nsmall=2)), 
             vjust = 0, hjust = 0.5, size=4)+
    #labs(subtitle = "Significance level: NS")+
    ylab("Relative impact")+
    ...
    theme(plot.title= element_text(hjust=0.01),
      plot.subtitle = element_text( hjust = 0.01))+
    scale_y_continuous(limits=c(min6+ min6/5, max5+max5/5))+
    scale_x_discrete(labels=c("Extra-\nEuropean", "Intra-\nEuropean", "Native", "Native\nalien elsewhere"))
...
plot <- ggarrange(Very_low_cover, Low_cover, Medium_cover, High_cover,
                  Very_high_cover, Extremely_high_cover, nrow=3, ncol=2, labels= c("a","b", "c","d","e","f"),
                  font.label = list(size = 12), align="hv")

plot
ggsave("../Images/Impact.png", plot= plot, width = 10, height = 10)

# save individually
#ggsave("../Images/Very_low.png", plot= Very_low_cover, width = 5, height = 3)
#ggsave("../Images/Low.jpg", plot= Low_cover, width = 5, height = 3)
#ggsave("../Images/Medium.jpg", plot= Medium_cover, width = 5, height = 3)
#ggsave("../Images/High.jpg", plot= High_cover, width = 5, height = 3)
#ggsave("../Images/Very_high.jpg", plot= Very_high_cover, width = 5, height = 3)
#ggsave("../Images/Extremely high.jpg", plot= Extremely high_cover, width = 5, height = 3)

```


```{r}
test <-eva[eva$name=="Digitaria sanguinalis",]
plots <- fullPlotData[fullPlotData$PlotObservationID %in% test$PlotObservationID,]
...
x$Neophyte[x$Neophyte=="native" & x$taxa %in% x$taxa[x$Neophyte=="intra"]] <- "native_intra"
...
## 3.2 Plot
```{r}
model <- lmer(RelDiff ~ Neophyte + (1|family/genus/taxa), x[x$class=="0%-1%",])
...
model <- lmer(RelDiff ~ Neophyte + (1|family/genus/taxa), x[x$class=="1%-10%",])
emm <- emmeans::emmeans(model, pairwise ~ Neophyte)
emm
plot(emmeans::emmeans(model, pairwise ~ Neophyte))
cld_results <- cld(emm$emmeans, alpha = 0.05, Letters = letters, adjust = "tukey", reversed=T)
...
model <- lmer(RelDiff ~ Neophyte + (1|family/genus/taxa), x[x$class=="10%-30%",])
emm <- emmeans::emmeans(model, pairwise ~ Neophyte)
emm
plot(emmeans::emmeans(model, pairwise ~ Neophyte))
cld_results <- cld(emm$emmeans, alpha = 0.05, Letters = letters, adjust = "tukey", reversed=T)
...
model <- lmer(RelDiff ~ Neophyte + (1|family/genus/taxa), x[x$class=="30%-50%",])
emm <- emmeans::emmeans(model, pairwise ~ Neophyte)
emm
plot(emmeans::emmeans(model, pairwise ~ Neophyte))
cld_results <- cld(emm$emmeans, alpha = 0.05, Letters = letters, adjust = "tukey", reversed=T)
...
model <- lmer(RelDiff ~ Neophyte + (1|family/genus/taxa), x[x$class=="50%-70%",])
emm <- emmeans::emmeans(model, pairwise ~ Neophyte)
emm
plot(emmeans::emmeans(model, pairwise ~ Neophyte))
cld_results <- cld(emm$emmeans, alpha = 0.05, Letters = letters, adjust = "tukey", reversed=T)
...
model <- lmer(RelDiff ~ Neophyte + (1|family/genus/taxa), x[x$class=="70%-100%",])
emm <- emmeans::emmeans(model, pairwise ~ Neophyte)
emm
plot(emmeans::emmeans(model, pairwise ~ Neophyte))
cld_results <- cld(emm$emmeans, alpha = 0.05, Letters = letters, adjust = "tukey", reversed=T)
...
p<-ggplot(Dataset, aes(x=Neophyte, y= RelDiff, group = Species,
    ...
    scale_fill_manual(values = c("#1E88E5", "#FFC107", "#004D40",  "darkgreen")) +
    theme(legend.position = "none")+
    theme(legend.text= element_text(size=12),legend.title =element_text(size=14))+
    annotate("text", x = 1, y = max1 + max1/10,label = "b", size = 4, vjust = 0.5, hjust = 0.5, alpha=1) +
    annotate("text", x = 2, y = max1 + max1/10, label = "b", size = 4, vjust = 0.5, hjust = 0.5, alpha=1)+
    annotate("text", x = 3, y = max1 + max1/10, label = "a", size = 4, vjust = 0.5, hjust = 0.5, alpha=1)+
    annotate("text", x = 4, y = max1 + max1/10, label = "ab", size = 4, vjust = 0.5, hjust = 0.5, alpha=1)+
    annotate("text", x=1, y=max1 + max1/5, 
             label= paste("n=",length(Dataset$Estimate[Dataset$Neophyte=="extra"])), 
             vjust = 0, hjust = 0.5, size=4)+
    annotate("text", x=2, y=max1 + max1/5, 
    ...
    annotate("text", x=3, y=max1 + max1/5, 
             label= paste("n=",length(Dataset$Estimate[Dataset$Neophyte=="native"])), 
             vjust = 0, hjust = 0.5, size=4)+
    annotate("text", x=4, y=max1 + max1/5, 
             label= paste("n=",length(Dataset$Estimate[Dataset$Neophyte=="native_intra"])), 
             vjust = 0, hjust = 0.5, size=4)+
    annotate("text", x=1, y=min2 + min2/5, 
             label= paste("mean=",format(round(mean(Dataset$RelDiff[Dataset$Neophyte=="extra"]), digits=2),  nsmall=2)), 
             vjust = 0, hjust = 0.5, size=4)+
    ...
    annotate("text", x=3, y=min2 + min2/5, 
             label= paste("mean=", format(round(mean(Dataset$RelDiff[Dataset$Neophyte=="native"]), digits= 2), nsmall=2)), 
             vjust = 0, hjust = 0.5, size=4)+
    annotate("text", x=4, y=min2 + min2/5, 
             label= paste("mean=", format(round(mean(Dataset$RelDiff[Dataset$Neophyte=="native_intra"]), digits= 2), nsmall=2)), 
             vjust = 0, hjust = 0.5, size=4)+
    #labs(subtitle = "Significance level: NS")+
    ylab("Relative impact")+
    ggtitle(title)+
...
p<-ggplot(Dataset, aes(x=Neophyte, y= RelDiff, group = Species,
    ...
    theme(legend.text= element_text(size=12),legend.title =element_text(size=14))+
    annotate("text", x = 1, y = max1 + max1/10,label = "b", size = 4, vjust = 0.5, hjust = 0.5, alpha=1) +
    annotate("text", x = 2, y = max1 + max1/10, label = "a", size = 4, vjust = 0.5, hjust = 0.5, alpha=1)+
    annotate("text", x = 3, y = max1 + max1/10, label = "a", size = 4, vjust = 0.5, hjust = 0.5, alpha=1)+
    annotate("text", x = 4, y = max1 + max1/10, label = "a", size = 4, vjust = 0.5, hjust = 0.5, alpha=1)+
    annotate("text", x=1, y=max1 + max1/5, 
             label= paste("n=",length(Dataset$Estimate[Dataset$Neophyte=="extra"])), 
             vjust = 0, hjust = 0.5, size=4)+
    annotate("text", x=2, y=max1 + max1/5, 
...
p<-ggplot(Dataset, aes(x=Neophyte, y= RelDiff, group = Species,
    ...
    theme(legend.text= element_text(size=12),legend.title =element_text(size=14))+
    annotate("text", x = 1, y = max1 + max1/10,label = "b", size = 4, vjust = 0.5, hjust = 0.5, alpha=1) +
    annotate("text", x = 2, y = max1 + max1/10, label = "ab", size = 4, vjust = 0.5, hjust = 0.5, alpha=1)+
    annotate("text", x = 3, y = max1 + max1/10, label = "a", size = 4, vjust = 0.5, hjust = 0.5, alpha=1)+
    annotate("text", x = 4, y = max1 + max1/10, label = "a", size = 4, vjust = 0.5, hjust = 0.5, alpha=1)+
    annotate("text", x=1, y=max1 + max1/5, 
             label= paste("n=",length(Dataset$Estimate[Dataset$Neophyte=="extra"])), 
...
p<-ggplot(Dataset, aes(x=Neophyte, y= RelDiff, group = Species,
    ...
    #annotate("text", x = 1, y = max1 + max1/10,label = "b", size = 4, vjust = 0.5, hjust = 0.5, alpha=1) +
    #annotate("text", x = 2, y = max1 + max1/10, label = "a", size = 4, vjust = 0.5, hjust = 0.5, alpha=1)+
    #annotate("text", x = 3, y = max1 + max1/10, label = "b", size = 4, vjust = 0.5, hjust = 0.5, alpha=1)+
    #annotate("text", x = 4, y = max1 + max1/10, label = "ab", size = 4, vjust = 0.5, hjust = 0.5, alpha=1)+
    annotate("text", x=1, y=max1 + max1/5, 
             label= paste("n=",length(Dataset$Estimate[Dataset$Neophyte=="extra"])), 
             vjust = 0, hjust = 0.5, size=4)+
...
plot
ggsave("../Images/Impact_native_intra.png", plot= plot, width = 10, height = 10)
```

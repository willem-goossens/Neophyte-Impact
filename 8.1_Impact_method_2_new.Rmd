...
```{r, warnings=F, message=F}
eva <- read_csv("../EVA data/fullPlotEva_ESy_1980.csv")
fullPlotData <- read_csv("../EVA data/fullPlotData_EUNIS_1980.csv")
...
if(native_intra_analysis){
  species_country_status<- read_csv("../EVA data/country_species_ESy_1980.csv", show_col_types = FALSE)
} else{
  species_country_status<- read_csv("../EVA data/country_species_ESy_1980.csv", show_col_types = FALSE)
...
ResultDominant_cover |> group_by(contrast) |> summarise(sign = sum(p.value < 0.05)/length(p.value))
...
p<-ggplot(Dataset, aes(x=Neophyte, y= RelDiff, group = Species,
    ...
    theme(legend.text= element_text(size=12),legend.title =element_text(size=14))+
    #annotate("text", x = 1, y = max5 + max5/10,label = "b", size = 4, vjust = 0.5, hjust = 0.5, alpha=1) +
    #annotate("text", x = 2, y = max5 + max5/10, label = "ab", size = 4, vjust = 0.5, hjust = 0.5, alpha=1)+
    #annotate("text", x = 3, y = max5 + max5/10, label = "a", size = 4, vjust = 0.5, hjust = 0.5, alpha=1)+
    annotate("text", x=1, y=max5 + max5/5, 
             label= paste("n=",length(Dataset$Estimate[Dataset$Neophyte=="extra"])), 
             vjust = 0, hjust = 0.5, size=4)+
...
#ggsave("../Images/Dominant.jpg", plot= Dominant_cover, width = 5, height = 3)

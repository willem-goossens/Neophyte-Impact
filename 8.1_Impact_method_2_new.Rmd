...
computePlantImpact <- function(plantName, plantStatus) {
  ...
  tmpFullPlotData <- tmpFullPlotData %>% 
    select(numberOfVascularPlantSpecies, Area, 
           ...
           Latitude, Longitude, Dataset, plantOccurs, 
           totalCover, speciesCover, relSpeciesCover)
...
x<- foreach(i= 1:length(allPlants$names), 
            .combine = "rbind", 
            .packages = c("dplyr","mgcv")) %dopar% {
  res <- computePlantImpact4(allPlants$names[i], allPlants$Neophyte[i])
}
end<-Sys.time()
round(end-begin, 2)

...
fullPlotData <- fullPlotData %>% 
    dplyr::select(PlotObservationID, ENS0, Area, 
           ...
           DistSeverity , Soil.Disturbance , 
           Grazing.Pressure , Mowing.Frequency , 
...
# load data
load(file = "I:/impact_new_again.Rdata") 
# plot data
plot(z)
...
# change names
plot_data$variable <-  c("Area", "EIVE M", "EIVE N", "EIVE R","EIVE L", "EIVE T", "DIV severity","DIV soil",
                         "DIV grazing","DIV mowing","HFP","MAP","Elevation","Intra","Extra","Spatial")

plot_data_intra$variable <-  c("Area", "EIVE M", "EIVE N", "EIVE R","EIVE L", "EIVE T", "DIV severity","DIV soil",
                         "DIV grazing","DIV mowing","HFP","MAP","Elevation","Spatial")
...
# classify in types
plot_data$type <- c(1,3,4,4,3,3,5,5,5,5,5,3,2,7,6,2)
plot_data$type <- as.factor(plot_data$type)
colnames(plot_data)[3] <- c("type")
plot_data <- plot_data %>% mutate(type = factor(type, 
...
# classify in types
plot_data_intra$type <- c(1,3,4,4,3,3,5,5,5,5,5,3,2,2)
...
# classify in types
plot_data_extra$type <- c(1,3,4,4,3,3,5,5,5,5,5,3,2,2)
...
# plot data
plot_native <- ggplot(plot_data, aes(x= reorder(variable, -value), y= value, fill=type))+
...
plot_native <- ggplot(plot_data, aes(x= reorder(variable, -value), y= value, fill=type))+
  ...
  theme(legend.position = "bottom")+
  theme(axis.text.x=element_text(size=12), axis.title.y=element_text(size=12), axis.title.x=element_blank(), 
        axis.text.y=element_text(size=10), plot.subtitle = element_text(size=12, face = "bold"))+
  labs(subtitle = "Native")
plot_native

plot_intra <- ggplot(plot_data_intra, aes(x= reorder(variable, -value), y= value, fill=type))+
  geom_bar(position= "stack",stat = "identity")+
  ggpubr::theme_pubr()+
  scale_x_discrete(guide = guide_axis(n.dodge = 2)) +
  NULL+
  ...
  theme(axis.text.x=element_text(size=12), axis.title.y=element_text(size=12), axis.title.x=element_blank(), 
        axis.text.y=element_text(size=10), plot.subtitle = element_text(size=12, face = "bold"))+
  labs(subtitle = "Intra-European alien")
plot_intra

plot_extra <- ggplot(plot_data_extra, aes(x= reorder(variable, -value), y= value, fill=type))+
  geom_bar(position= "stack",stat = "identity")+
  ggpubr::theme_pubr()+
  scale_x_discrete(guide = guide_axis(n.dodge = 2)) +
  NULL+
  ...
  theme(axis.text.x=element_text(size=12), axis.title.y=element_text(size=12), axis.title.x=element_blank(), 
        axis.text.y=element_text(size=10), plot.subtitle = element_text(size=12, face = "bold"))+
  labs(subtitle ="Extra-European alien")
plot_extra

all <- ggarrange(plot_native, plot_extra, plot_intra, common.legend = T, align="hv", ncol=1, nrow=3, legend= "bottom", labels= letters[1:3])
all

ggsave("../Images/variance_all.png", width=8, height=8)
...
# create vector for variables
variables = data.frame(var = c("EIVEresT","EIVEresN","EIVEresM","EIVEresR","EIVEresL","DistSeverity","Soil.Disturbance",
                               "Grazing.Pressure","Mowing.Frequency", "hfp","DI_extra", "DI_intra"),
                 type= c(3,4,3,4,3,5,5,5,5,5,6,7), 
...
create_plot= function(name){
    ...
    plot_end <- ggplot() +
      ...
      scale_colour_manual(name="Origin", values=c("Extra-European"= "#004D40","Intra-European" ="#FFC107","Native"="#1E88E5"))+
      scale_fill_manual(name="Origin", values=c("Extra-European"= "#004D40","Intra-European" ="#FFC107","Native"="#1E88E5"))+ 
      scale_y_continuous(
        name = "Native SR",
...
# function to create plot
create_plot_alien2= function(name, name2){
    model_name <-  variables[variables$name == name,1]
    col<- variables[variables$name == name,4]
    ...
    model_name2 <-  variables[variables$name == name2,1]
    col<- variables[variables$name == name2,4]
    
    plot_pred_nat2 <- ggeffects::predict_response(mod, model_name2)
    
    plot_pred_nat2$Neophyte <- ifelse(name2=="APP intra-European", "Intra-European", "Extra-European")
    ...
    plot_end <- ggplot() +
      ...
      geom_line(data = plot_pred_nat, aes(x = x, y = predicted, color = Neophyte), linewidth = 1.2) +
      geom_ribbon(data = plot_pred_nat, aes(x = x, ymin = conf.low, ymax = conf.high, fill = Neophyte), alpha = 0.2) +
      geom_line(data = plot_pred_nat2, aes(x = x, y = predicted, color = Neophyte), linewidth = 1.2) +
      geom_ribbon(data = plot_pred_nat2, aes(x = x, ymin = conf.low, ymax = conf.high, fill = Neophyte), alpha = 0.2) +
      scale_colour_manual(values=c("Extra-European"= "#004D40","Intra-European" ="#FFC107","Native"="#1E88E5"))+
      scale_fill_manual(values=c("Extra-European"= "#004D40","Intra-European" ="#FFC107","Native"="#1E88E5"))+ 
      scale_y_continuous(
        name = "Native SR" ) +
      ...
      ylab(analysis) +
      theme(legend.position = "none")+
      scale_x_continuous(name= "Alien plant prevalence")
}
...
plot_both <- create_plot_alien2("APP intra-European","APP extra-European")
...
#ggsave("../Images/Variance.svg", width=8, height=8)
```


```{r}
leg <- ggpubr::get_legend(plot_EIVE_N)


under <- ggarrange(ggarrange(plot_EIVE_N, plot_EIVE_R, plot_hfp, plot_sev,  plot_intra, plot_extra, ncol=2, nrow=3, labels= letters[2:7], 
                                 common.legend = T,legend="none", align="hv"), 
                                 ggarrange(plot_daisie, nrow=1, ncol=1, legend= "none", labels= c("h")),ncol=1, nrow=2,
                                 legend.grob = leg, heights= c(1,1,1), common.legend = T, align="hv", legend="bottom")

under <- ggarrange(plot_EIVE_N, plot_EIVE_R, plot_hfp, plot_sev,  plot_both, plot_daisie, ncol=2, nrow=3, labels= letters[2:7], 
                                 common.legend = T, align="hv", legend.grob = leg, heights= c(1,1,1), legend="bottom")
under

plot_all <-  ggarrange(plot_variance, under,ncol=1, nrow=2,
                                 labels= letters[1], heights= c(1,2))
plot_all
#ggsave("../Images/Variance_daisie.svg", width=8, height=8)
ggsave("../Images/Variance_daisie.png", width=8, height=8)
...
# create vector for variables
variables = data.frame(var = c("EIVEresT","EIVEresN","EIVEresM","EIVEresR","EIVEresL","DistSeverity","Soil.Disturbance",                               "Grazing.Pressure","Mowing.Frequency", "hfp","DI_extra", "DI_intra", "Area","chelsaP","elev","Longitude","Latitude"),
                 type= c(2,3,2,3,2,4,4,4,4,4,5,6,1,2,1,1,1), 
                 name = c("EIVE T", "Soil nitrogen (EIVE)", "EIVE M","Soil reaction (EIVE)", "EIVE L", "Disturbance severity (DIV)", 
                          "Soil disturbance (DIV)","Grazing pressure (DIV)","Mowing pressure (DIV)","Human Footprint",
                          "APP extra-European", "APP intra-European","ln(Area)",
                          "Mean annual precipitation","Elevation","Longitude","Latitude"))
variables$color <-my_palette[variables$type]

plot_area <- create_plot("ln(Area)")
plot_EIVE_L <- create_plot("EIVE L")
plot_EIVE_T <- create_plot("EIVE T")
plot_EIVE_M <- create_plot("EIVE M")
plot_MAP <- create_plot("Mean annual precipitation")
plot_elevation<- create_plot("Elevation")
plot_longitude<- create_plot("Longitude")
plot_latitude<- create_plot("Latitude")
plot_soil<- create_plot("Soil disturbance (DIV)")
plot_grazing<- create_plot("Grazing pressure (DIV)")
plot_mowing<- create_plot("Mowing pressure (DIV)")

blank_plot <- ggplot()+ geom_blank() +theme_classic()

leg <- ggpubr::get_legend(plot_area)


appendix <- ggarrange(ggarrange(plot_area, plot_elevation,plot_longitude, plot_latitude,
                                plot_MAP, plot_EIVE_M, plot_EIVE_L, plot_EIVE_T, plot_soil, plot_grazing, 
                                ncol=2, nrow= 5, align="hv",  legend="none", labels= letters[1:10], widths = c(1,1)),
                       ggarrange(blank_plot, plot_mowing, blank_plot,labels= c("","k",""), widths= c(0.5,1,0.5), 
                                 nrow=1, ncol=3, legend= "none"),
                       ncol=1, nrow=2, heights = c(5,1), common.legend = T,legend="bottom", align="hv", legend.grob = leg)

ggsave("../Images/Appendix_variables.png", plot= appendix, width=10, height= 12)
...
ggsave(EU, file= "../Images/Degree_invasion_EU.png", width=8, height=6)
```
...
data <- spatSample(hfp2009, 10000, na.rm=T, method= "random")

eva <- as.data.frame(sample(fullPlotData$hfp, 10000))
colnames(eva) <- "HFP2009"


data$source <- "Europe"
eva$source <- "EVA plots"

colnames(data)
colnames(eva)


combined <- rbind(data, eva)

# If you want to use a t-test (assumes normality)
t_test_result <- t.test(HFP2009 ~ source, data = combined)
print(t_test_result)

plot<- ggplot(combined, aes(x = source, y = HFP2009,fill = source)) +
  geom_violin(trim = FALSE) +
  theme_minimal() +
  labs(y = "Human Footprint (HFP)", x = "Plots")+
  geom_boxplot(width= 0.2, alpha=0.75, fill="white", outliers=F)+
  theme_pubr()+
  stat_summary(fun= "mean",
                 geom = "point", aes(group= source), size=3)+
  xlab(NULL)+
  theme(legend.position = "none")+
  annotate("text", x = 1, y = max(combined$HFP2009[combined$source=="Europe"])+max(combined$HFP2009[combined$source=="Europe"])/5,
           label = "b", size = 4, vjust = 0.5, hjust = 0.5, alpha=1) +
  annotate("text", x = 2, y = max(combined$HFP2009[combined$source=="EVA plots"])+ max(combined$HFP2009[combined$source=="Europe"])/5,
           label = "a", size = 4, vjust = 0.5, hjust = 0.5, alpha=1)

plot

ggsave("../Images/HFP_Europe.png", plot= plot, width = 6, height = 3)
```

```{r}
colnames(medRegions)
unique(medRegions$Region)
germany <- medRegions[medRegions$Region=="Belgium",]
germany<- germany[!is.na(germany$OBJECTID_1),]

hfp_germany <- mask(hfp2009, germany)
hfp_germany <- crop(hfp_germany, germany)

data <- spatSample(hfp_germany, 10000, na.rm=T, method= "random")
...
```{r}
regions <- unique(medRegions$Region)

factor <- length(fullPlotData$PlotObservationID)/10000

data_sampled <- data.frame()
i <- 2
for(i in 1:length(regions)){
  name <- regions[i]
  length <- ceiling(sum(fullPlotData$Region==name)/factor)
  
  if(length == 0) next
  country <- medRegions[medRegions$Region==name,]
  country<- country[!is.na(country$OBJECTID_1),]

  hfp_country <- mask(hfp2009, country)
  hfp_country <- crop(hfp_country, country)
  
  data <- spatSample(hfp_country, length, na.rm=T, method= "random")
  
  data$eva <- sample(fullPlotData$hfp, nrow(data))
  data$country <- name
  
  data_sampled <- rbind(data_sampled, data)
  
}


eva <- as.data.frame(sample(fullPlotData$hfp, nrow(data_sampled)))
colnames(eva) <- "HFP2009"

data_sampled$source <- "Europe"
eva$source <- "EVA plots"

...
```{r}
taxaForCoverClassAnalysis = read_csv("taxa_new.csv", show_col_types = FALSE)
length(taxaForCoverClassAnalysis$taxa.names)
...
## 2.2 Base model
We compute our base model with the best solution (see [rmarkdown link](6_Base_Model.Rmd)), which includes smoothers for all EIVE without weighting, smoothers for the DIV, a smoother for log(area), a smoother for spatial autocorrelation and the dataset as a random effect. We use a poisson distribution with a log link function. 
```{r}
...
computePlantImpact2 <- function(plantName, status) {
  ...
  # Make empty dataframe to store the results. We want to have the species name, mean residuals per cover, total number of plots. The log defines the method applied
  res<- result |> group_by(coverClass) |> summarise(taxa= plantName, absent = meanRel[1], present = meanRel[2], low = meanRel[3], medlow = meanRel[4], med = meanRel[5], medhigh = meanRel[6], dominant = meanRel[7],
                 numberOfPlots = sum(!(tmpFullPlotData$coverClass == "0%")))  
  ...
  # Add the findings to the dataframe
  res <- add_row(res, taxa = plantName, log = "1",
                 absent = result$meanRel[1], present = result$meanRel[2], low = result$meanRel[3], medlow = result$meanRel[4], med = result$meanRel[5], medhigh = result$meanRel[6], dominant = result$meanRel[7],
                 numberOfPlots = sum(!(tmpFullPlotData$coverClass == "0%")), status= status)
  res <- add_row(res, taxa = plantName, log ="2",
                 absent = result$meanLogRel[1], present = result$meanLogRel[2], low = result$meanLogRel[3], medlow = result$meanLogRel[4], med = result$meanLogRel[5], medhigh = result$meanLogRel[6], dominant = result$meanLogRel[7],
                 numberOfPlots = sum(!(tmpFullPlotData$coverClass == "0%")), status= status)
  res <- add_row(res, taxa = plantName, log ="3",
                 absent = result$meanRR[1], present = result$meanRR[2], low = result$meanRR[3], medlow = result$meanRR[4], med = result$meanRR[5], medhigh = result$meanRR[6], dominant = result$meanRR[7],
                 numberOfPlots = sum(!(tmpFullPlotData$coverClass == "0%")), status= status)
  res
}
```
...
```{r}
intra<- impact[impact$status=="intra",]
```
...
p<- ggplot(summarizedData[summarizedData$log=="3",], aes(x=cover_class, y=(exp(mean)-1)*100, group=status, color=status)) + 
  ...
  xlab("Cover Class") +
  scale_x_discrete(labels = c('0% (absent)','[0%-1%]','[1% - 10%]', '[10%-30%]','[30%-50%]', '[50%-70%]','[50%-100%]')) +    
...
p<- ggplot(summarizedData[summarizedData$log=="3",], aes(x=cover_class, y=mean, group=status, color=status)) + 
  ...
  scale_y_continuous(limits = c(-0.40, 0.30))+
  scale_colour_manual(values=c("#1E88E5", "#FFC107", "#004D40"), 
                    name="Legend",
                    breaks=c("native", "intra","extra"),
                    labels=c("native species", "intra European neophyte", "extra European neophyte"))+

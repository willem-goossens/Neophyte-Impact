...
MDL <- glmer(ENS0 ~ EIVEresN+ cover_rel_intra + cover_rel_extra+ (1| Dataset), data=fullPlotData, family= poisson(link=log))
summary(MDL)

visreg::visreg(MDL, gg = TRUE, partial=FALSE, rug = F,  scale="response")

# scale the work to 0-1 scale
scaled <-fullPlotData

scale_values <- function(x){(x-min(x))/(max(x)-min(x))}

scaled[, c(4:12, 16:18)] <- apply(scaled[, c(4:12, 16:18)], 2, scale_values)

MDL <- glmer(ENS0 ~ EIVEresN + cover_rel_intra + cover_rel_extra+ (1| Dataset), data=scaled, family= poisson(link=log))
summary(MDL)

visreg::visreg(MDL, scale="response", partial=T)

plot(scaled$EIVEresN, scaled$ENS0)
...
cor <- cor(fullPlotData[, c(2, 4:14, 16:34)], use="pairwise.complete.obs")
...
plot(z, ylim=c(0,50))      
points(fullPlotData$EIVEresN, fullPlotData$ENS0)

z <-visreg::visreg(baseModel, xvar = "cover_rel_intra", gg = TRUE, partial=T, rug = F,  scale="response", plot=F)

plot(z, ylim=c(0,50))
points(fullPlotData$cover_rel_intra, fullPlotData$ENS0)

plot(lm(ENS0~ EIVEresN, fullPlotData))
z<-lm(ENS0~ EIVEresN + I(EIVEresN^2), fullPlotData)
summary(z)
...
MDL <- glmer(ENS0 ~ EIVEresN+hfp+ cover_rel_extra + cover_rel_intra+ (1|Dataset), data= scaled, family= poisson(link=log))

MDL <- glmer(ENS0 ~ EIVEresN+ 
                hfp + cover_rel_intra+  (1| Dataset),
                data= scaled, family= poisson(link=log))

MDL_dredge<-dredge(MDL, rank = AICc, options(na.action = na.fail))
options(na.action = "na.omit")
subset_models <- get.models(MDL_dredge, delta < 4)

model_average <- model.avg((subset_models), fit=T )
summary(model_average)
MDL_AVG<- list(model_average)
```
...
visreg::visreg(baseModel, gg = TRUE, partial=FALSE, rug = F,  scale="response")
...
plot_model(baseModel, type="pred", terms = c("rel_n_intra [all]"))

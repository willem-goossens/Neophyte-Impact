...
# classify in types
plot_data$type <- c(1,2,2,2,2,2,3,3,3,3,3,2,2,4,5)
...
EU <-ggplot() +
 geom_tile(data = bathy_df, aes(x = x, y = y, fill = elevation)) +
  ...
  new_scale_fill() +
  geom_raster(data = hfp_df, aes(x = x, y = y, fill = hfp)) +
  ...
  theme(
    ...
    plot.margin = margin(5, 5, 5, 5)# Remove the legend
  )+
  # Theme customization
  theme_minimal() +
  labs(fill = "Mean degree \nof invasion [%]",
       x = "Longitude",
       y = "Latitude")
EU

#ggsave(EU, file= "../Images/Degree_invasion_EU.jpg", width=10, height=7)
```

...
if(normal){
  ...
  fullPlotData_EUNIS <- read_csv("fullPlotData_EUNIS_1980.csv")
  fullPlotData$Date <- as.Date(fullPlotData$Date, format = c("%Y-%m-%d"))
  fullPlotData <- fullPlotData[fullPlotData$Date > "1980-01-01",]
...
#write_csv(speciesDominance, "speciesDominance_1980.csv")
} else{
  if(native_intra_analysis){
  speciesDominance <- read.csv("speciesDominance_native_intra.csv")
  }else{
  speciesDominance<- read.csv("speciesDominance_1980.csv")
  }
}
```
...
```{r}
phylo <- read_csv("phylo.csv", show_col_types = F)
...
group_counts <- table(speciesDominance$neophyte)

# try weighting based on amount of observations
speciesDominance$weights <- 1 / group_counts[speciesDominance$neophyte]
speciesDominance$weights <- speciesDominance$weights / mean(speciesDominance$weights)  # Normalize around 1

# glmm
model <- lmer(log10(coverMean) ~ -1+ neophyte + (1|family/genus), data= speciesDominance, weights = weights )
summary(model)
...
Dominance <- p
p
ggsave("Dominance.jpeg", p,width = 5, height = 5)
}
...
# glmm
model <- lmer(log10(numberOfPlots) ~ -1+ neophyte + (1|family/genus), data= speciesDominance, weights = weights )
summary(model)
...
Frequency <- p
p
#ggsave("Frequency.jpeg", p,width = 5, height = 5)
} else {
...
plot <- ggarrange(Dominance, Frequency, nrow=1, ncol=2, labels=c("a","b"), font.label=list(size=12), align="hv")
plot
#ggsave("Dominance_frequency.svg", plot= plot, width = 8, height = 4)
```

...
speciesDominance<-foreach(i= 1:length(uniqueNative), .combine='rbind', .packages=c("dplyr","mgcv")) %dopar% {
  ...
  if(length(tmp$PlotObservationID) >= 10) {
...
p
ggsave("Dominance New.jpeg", p,width = 6, height = 4)
```
...
p<- ggplot(speciesDominance, mapping = aes(x= neophyte, y = log10(numberOfPlots), colour = neophyte)) + 
  ...
  annotate("text", x = 1, y = mean(log10(speciesDominance$numberOfPlots[speciesDominance$neophyte=="extra"])), label = "a", size = 3.5, vjust = -2, hjust = 0.5) +
  annotate("text", x = 2, y = mean(log10(speciesDominance$numberOfPlots[speciesDominance$neophyte=="intra"])), label = "b", size = 3.5, vjust = -2, hjust = 0.5)+
...
p
ggsave("Frequency New.jpeg", p,width = 6, height = 4)
```

# 4 Export
```{r}
#write.csv(speciesDominance, "speciesDominance_new_categorization_10.csv")
```

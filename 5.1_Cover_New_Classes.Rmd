...
```{r}
# run for intra-European analysis

intra <- 	speciesDominance$names[speciesDominance$neophyte=="intra"]

speciesDominance$neophyte[speciesDominance$names %in% intra & speciesDominance$neophyte=="native"] <- "native_intra"
...
# glmmTMB
speciesDominance$rel <- speciesDominance$coverMean/100
model <- glmmTMB((rel) ~ -1+ neophyte + (1|family/genus/names), data= speciesDominance, family= beta_family() )
summary(model)
...
while (successful_runs < 1000) {
  ...
  if(native_intra_analysis){
    ...
    native_intra <- native_intra[sample(nrow(native_intra),200, replace=T),]
...
topdominant_alien <- topdominant[topdominant$neophyte != "native",]
library(readxl)
write_excel_csv2(topdominant_alien, "../Results/speciesDominance_top100_aliens.csv")
```
...
# glmmTMB
model <- glmmTMB((numberOfPlots) ~ -1+ neophyte + (1|family/genus/names), data= speciesDominance, family= nbinom2() )
summary(model)
summary(emmeans::emmeans(model, pairwise ~ neophyte))
...
Frequency <- ggplot(speciesDominance, aes(x=neophyte, y=log10(numberOfPlots), group=Neophyte, colour=Neophyte, fill=Neophyte)) + 
  ...
  # Significance letters (lower position)
  annotate("text", x=1, y=y_text_Frequency, label="c", size=4, hjust=0.5, alpha=1) +
  annotate("text", x=2, y=y_text_Frequency, label="c", size=4, hjust=0.5, alpha=1) +
  annotate("text", x=3, y=y_text_Frequency, label="b", size=4, hjust=0.5, alpha=1)+
  annotate("text", x=4, y=y_text_Frequency, label="a", size=4, hjust=0.5, alpha=1)
}
Frequency
Dominance
# Arrange both plots in one figure
plot <- ggarrange(Dominance, Frequency, nrow=1, ncol=2, align="hv", labels=c("a","b"))
plot


ggsave("../Images/Dominance_frequency_native_intra.png", plot= plot, 
       width = 10, height = 5)
...
eva[eva$name=="Rumex patientia aggr.",]

fullPlotData[fullPlotData$PlotObservationID %in% eva$PlotObservationID[eva$name=="Rumex patientia aggr."],]
(eva$name[eva$PlotObservationID %in% c(2195066)])
(eva$name[eva$PlotObservationID %in% c(2195067)])

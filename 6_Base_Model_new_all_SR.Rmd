...
correlation <- cor(NaRV.omit(fullPlotData[,c(9,11:58)]))
...
model6<-  ENS0 ~ 
              ...
              s(Latitude, Longitude, bs = 'tp') +
              s(Dataset, bs = 're')
gam12 <- bam(model2,family = poisson(link = log), data = fullPlotData,  method = 'fREML',  discrete=TRUE, nthreads=4)
gam13 <- bam(model3,family = poisson(link = log), data = fullPlotData,  method = 'fREML',  discrete=TRUE, nthreads=4)
gam14 <- bam(model4,family = poisson(link = log), data = fullPlotData,  method = 'fREML',  discrete=TRUE, nthreads=4)
gam15 <- bam(model5,family = poisson(link = log), data = fullPlotData,  method = 'fREML',  discrete=TRUE, nthreads=4)
gam16 <- bam(model6,family = poisson(link = log), data = fullPlotData,  method = 'fREML',  discrete=TRUE, nthreads=4)
summary(gam12)
summary(gam13)
summary(gam14)
summary(gam15)
summary(gam16)
AIC(gam1,gam3,gam4,gam5,gam6,gam7,gam8,gam9,gam10,gam11,gam12, gam13,gam14,gam15)
...
gam11 <- bam(model6,family = quasipoisson, data = fullPlotData,  method = 'fREML',  discrete=TRUE, nthreads=4)
...
visreg(gam11, xvar = "elev", ylab = "Richness", xlab = "elev", gg = TRUE, partial=FALSE, rug = FALSE,  scale="response") 
